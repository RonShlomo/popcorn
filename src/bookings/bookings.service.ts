  ...
  async create(newBooking: Prisma.BookingUncheckedCreateInput) {
    ...
    if (!showtime)
      throw new NotFoundException(
        `Showtime with id "${newBooking.showtimeId}" not found`,
      );

    //Check if the seatNumber already exists in the showtime's bookings
    if (
      showtime?.bookings?.some(
        (booking) => booking.seatNumber === newBooking.seatNumber,
      )
    ) {
      throw new ConflictException(
        `Seat number ${newBooking.seatNumber} is already booked for this showtime.`,
      );
    }
    ...
    if (startTime < now) {
      throw new BadRequestException(
        'The start time of this showtime has already passed',
      );
    }

    const book = await this.databaseService.booking.create({
      data: newBooking,
    });
